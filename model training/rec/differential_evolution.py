...
class DifferentialEvolutionSolver(object):
    ...
    def solve(self):
        ...
        # Data Collection for ML hyper-parameter tuning
        DE_result.data = np.zeros((100, 11))
        DE_result.replacements = np.zeros((100,))
        DE_result.best_updates = np.zeros((100,))
        d_step = len(self.data)//100
        for d_idx in range(0, 99, ):
            DE_result.data[d_idx, :] = np.array(self.data[d_idx*d_step])
            DE_result.replacements[d_idx] = np.average(self.replace_ratio[d_idx*d_step:(d_idx+1)*d_step])
            DE_result.best_updates[d_idx] = np.average(self.best_update[d_idx*d_step:(d_idx+1)*d_step])
        DE_result.data[99] = np.array(self.data[len(self.data)-1])
        DE_result.replacements[99] = np.average(self.replace_ratio[99*d_step:len(self.replace_ratio)])
        DE_result.best_updates[99] = np.average(self.best_update[99*d_step:len(self.best_update)])
    ...
    def __next__(self):
        ...
        if self._updating == 'immediate':
            ...
            f_pop = np.zeros((self.num_population_members,))
            replacements = 0
            best_replacement = 0
            for candidate in range(self.num_population_members):
                ...
                if self._accept_trial(energy, feasible, cv,
                    ...
                    if self._accept_trial(energy, feasible, cv,
                        ...
                        self._promote_lowest_energy()
                        best_replacement = 1
            self.data.append(np.percentile(f_pop, range(0, 110, 10)))
            self.replace_ratio.append(replacements/self.num_population_members)
            self.best_update.append(best_replacement)
